      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="space-y-4 sm:space-y-5">
          {mockMakers.map(maker => (
            <div key={maker.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 sm:p-6 hover:border-orange-500 transition-colors">
              {/* Maker Header */}
              <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-4 sm:mb-6 gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg sm:text-xl font-semibold">{maker.name}</h2>
                    <p className="text-xs sm:text-sm text-gray-400">Port: {maker.port}</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Link to={`/makerDetails/${maker.id}`} className="px-3 sm:px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-all text-sm font-semibold">
                    Manage
                  </Link>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 mb-4 sm:mb-6">
                <div className="bg-gray-800 p-3 rounded-lg">
                  ...
                  <div className="text-sm sm:text-base font-semibold text-orange-500">{maker.balance} BTC</div>
                </div>
                <div className="bg-gray-800 p-3 rounded-lg">
                  <div className="text-xs text-gray-400 mb-1">Active Swaps</div>
                  <div className="text-sm sm:text-base font-semibold text-blue-500">{maker.activeSwaps}</div>
                </div>
                <div className="bg-gray-800 p-3 rounded-lg">
                  <div className="text-xs text-gray-400 mb-1">Earnings</div>
                  <div className="text-sm sm:text-base font-semibold text-emerald-500">{maker.earnings} BTC</div>
                </div>
                <div className="bg-gray-800 p-3 rounded-lg">
                ...
                <div className="bg-gray-800 p-3 rounded-lg col-span-2">
                  <div className="text-xs text-gray-400 mb-1">Data Directory</div>
                  <div className="text-xs sm:text-sm font-mono text-gray-300 truncate">{maker.dataDir}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 pt-4 border-t border-gray-800">
                <div>
                  ...
                  <div className="text-xs sm:text-sm font-mono text-gray-300">{maker.bitcoinRpc}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-1">Tor Address</div>
                  <div className="text-xs sm:text-sm font-mono text-gray-300 truncate">{maker.torAddress}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        {mockMakers.length === 0 && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-12 text-center">
            ...
            <p className="text-gray-400 mb-6">Get started by adding your first maker instance</p>
