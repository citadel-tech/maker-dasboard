...
export default function Logs({ id }: Props) {
  ...
  useEffect(() => {
    monitoring.dataDir(id).then(setDataDir).catch(() => {});
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          {LEVELS.map((lvl) => {
            ...
            return (
              ...
              >
                {lvl}
                <span className={`ml-1.5 ${active ? "opacity-80" : "text-gray-500"}`}>
                  {count}
                </span>
              </button>
            );
      ...
      {logPath && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="text-xs font-mono text-gray-300 truncate">{logPath}</p>
          </div>
          <button
            type="button"
            onClick={copyPath}
