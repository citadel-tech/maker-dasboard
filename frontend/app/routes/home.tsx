...
export default function Home() {
  // Calculate totals
  const totalBalance = mockMakers
    .reduce((sum, m) => sum + parseFloat(m.balance), 0)
    .toFixed(2);
  const totalSwaps = mockMakers.reduce((sum, m) => sum + m.activeSwaps, 0);
  const totalEarnings = mockMakers
    .reduce((sum, m) => sum + parseFloat(m.earnings), 0)
    .toFixed(4);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mb-6 sm:mb-8">
          <div className="bg-gray-900 p-4 sm:p-5 rounded-xl border border-gray-800">
            ...
            <div className="text-xs text-gray-500 mt-1">
              Across {mockMakers.length} makers
            </div>
          </div>
          ...
          <div className="bg-gray-900 p-4 sm:p-5 rounded-xl border border-gray-800">
            ...
            <div className="text-2xl sm:text-3xl font-bold text-purple-500">
              {onlineMakers}/{mockMakers.length}
            </div>
        ...
        <div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 sm:p-5">
            {mockActivity.map((activity) => (
              <div
                key={activity.id}
                className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-3 sm:py-4 border-b border-gray-800 last:border-b-0 gap-2 sm:gap-0"
              >
                <div className="flex flex-col gap-1">
                  <div className="text-sm text-gray-100">{activity.type}</div>
                  <div className="text-xs text-gray-400 break-all">
                    {activity.maker} • {activity.details}
                  </div>
                </div>
